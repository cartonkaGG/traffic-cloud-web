...
import { loadFfmpeg, writeInputFile, readOutputBlob, cleanupFiles } from '@/lib/videoUniquify/ffmpegClient'

import { runServerUniquify, fetchServerUniquifyStatus } from '@/lib/videoUniquify/serverUniquifyClient'
...
const SPECS = [
  ...
  { icon: Shield, label: 'Серверна обробка' },
...
export function VideoUniquifyPage(): JSX.Element {
  ...
  useEffect(() => {
    ...
    void fetchServerUniquifyStatus().catch(() => {})
  ...
  const run = useCallback(async () => {
    ...
    try {
      ...
      try {
        ...
        const serverItems = await runServerUniquify({
        ...
        out = serverItems.map((item, i) => ({
          ...
          sizeMb: item.blob.size / (1024 * 1024)
        ...
        if (
          ...
          serverMsg.includes('підписк') ||

          serverMsg.includes('Максимум') ||

          serverMsg.includes('копій') ||

          serverMsg.includes('Забагато запитів')
        ...
        setProgress('Сервер недоступний — пробуємо у браузері…', 6)
  ...
  const loaderSublabel = progressLabel?.includes('браузер')

    ? 'Резервний режим у браузері. Перший запуск може зайняти кілька секунд.'

    : 'Не закривайте вкладку. Серверна обробка зазвичай швидша за браузер.'
          ...
          <div className="relative flex flex-wrap items-start gap-5">
            ...
            <div className="min-w-0 flex-1">
              ...
              <p className="mt-3 max-w-2xl text-sm leading-relaxed text-zinc-400">
                ...
                Одне відео — десятки унікальних копій. За замовчуванням обробка на сервері Traffic

                Cloud (без WebAssembly у браузері). Кожна версія з новим кропом, кольором, темпом і
