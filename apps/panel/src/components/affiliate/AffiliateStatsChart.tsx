        ...
        <div className="mt-4 flex h-28 items-end justify-between gap-1.5">
          {daily.map((d) => (
            <div key={d.date} className="flex min-w-0 flex-1 flex-col items-center gap-2">
              <div
                className="w-full max-w-[2rem] rounded-t-md bg-gradient-to-t from-cyan-500/20 to-cyan-400/70"
                style={{ height: `${Math.max(6, (d.joins / maxJoins) * 100)}%` }}
