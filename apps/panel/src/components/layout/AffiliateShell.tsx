...
const ROUTE_META: Record<string, { title: string; kicker: string }> = {
  '/affiliate': { title: 'Офери та статистика', kicker: 'Залив трафіку' },
        ...
        <>
          {isActive ? (
            <motion.span
              ...
              className="absolute inset-0 rounded-xl bg-gradient-to-r from-emerald-500/15 to-transparent"
...
export function AffiliateShell(): JSX.Element {
  ...
  const meta = ROUTE_META[pathname] ?? ROUTE_META['/affiliate']

  const mainNav: NavItem[] = [
    { to: '/affiliate', label: 'Офери та статистика', icon: TrendingUp, end: true }
  ]
    ...
    <div className="flex h-full min-h-0 bg-ink">
      <Sidebar>
        <div className="mb-8 px-2">
          <PanelBrand layout="sidebar" homeTo="/affiliate" />
        </div>
