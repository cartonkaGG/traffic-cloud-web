...
export function AccountsScreen() {
  return (
    <OutreachChrome
      active="accounts"
      path="/accounts"
      kicker="Session · MTProto"
      title="Акаунти Telegram"
    >
      <div style={{ display: 'flex', justifyContent: 'space-between', marginBottom: 14 }}>
        <span style={{ fontSize: 12, color: '#64748b' }}>MTProto-сесія · проксі SOCKS5 · health</span>
        <div
          style={{
            fontSize: 11,
            fontWeight: 600,
      ...
      <div style={{ position: 'relative' }}>
        <HighlightRing x={0} y={0} width={780} height={92} label="MTProto + SOCKS5 проксі" />
        {ACCOUNTS.map((a, i) => (
          <div
            key={a.label}
            style={{
              marginBottom: i < ACCOUNTS.length - 1 ? 10 : 0,
              padding: '12px 14px',
              borderRadius: 12,
              border: i === 0 ? '1px solid rgba(94,200,255,0.25)' : '1px solid rgba(255,255,255,0.08)',
              background: i === 0 ? 'rgba(94,200,255,0.06)' : 'rgba(255,255,255,0.03)'
            }}
          >
            <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'flex-start' }}>
              <div>
                ...
                <div style={{ fontSize: 11, color: '#64748b', marginTop: 4, fontFamily: 'ui-monospace' }}>
                  {a.phone} · {a.proxy}
                </div>
              </div>
